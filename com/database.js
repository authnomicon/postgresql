  ...
  return new Promise(function(resolve, reject) {
    ...
    client.connect(function(err) {
      ...
      this.query('SELECT to_regclass($1::text)', [ 'users' ])
        ...
        .then(function(res) {
          return self.query('SELECT to_regclass($1::text)', [ 'client' ])
            .then(function(res) {
              if (res && res.rows && res.rows[0] && res.rows[0]['to_regclass'] === null) {
        ...
        .then(function(res) {
          return self.query('SELECT to_regclass($1::text)', [ 'grant' ])
            .then(function(res) {
              if (res && res.rows && res.rows[0] && res.rows[0]['to_regclass'] === null) {
                var sql = fs.readFileSync(path.join(__dirname, '../lib/schema/grant.sql'), 'utf8');
                return self.query(sql);
              }
            });
        })
