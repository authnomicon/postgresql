var PublicKeyStore = require('../../lib/credentials/publickeystore');
var fs = require('fs');
  ...
  return pool.query('SELECT to_regclass($1::text)', [ 'public_key_credentials' ])
    .then(function(res) {
      if (res && res.rows && res.rows[0] && res.rows[0]['to_regclass'] === null) {
        ...
        console.log('creating public key credentials table in: ');
      
        var sql = fs.readFileSync(path.join(__dirname, '../../lib/schema/public_key_credentials.sql'), 'utf8');
        return pool.query(sql);
      }
    })
    .then(function() {
      return new PublicKeyStore(pool);
    });
};
